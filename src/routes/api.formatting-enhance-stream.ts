...
import { FormattingEnhanceInput, buildEnhancePrompt } from "@/lib/formatting.functions";
import { checkQuota } from "@/lib/usage-tracking";
import { supabaseAdmin } from "@/integrations/supabase/client.server";
import type { Database } from "@/integrations/supabase/types";

export const Route = createFileRoute("/api/formatting-enhance-stream")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const quota = await checkQuota(supabaseAdmin, userId, "formatting");
        if (!quota.allowed) {
          return new Response(
        ...
        const parsed = FormattingEnhanceInput.safeParse(body);
        if (!parsed.success) {
        ...
        const { promptCached, promptDynamic } = buildEnhancePrompt(parsed.data);
        ...
        try {
          const result = streamText({
            ...
            onError: ({ error }) => {
              console.error("[formatting-enhance-stream] generation error:", error);
            },
          });
          return toTextStreamResponseWithErrors(result);
        } catch (e) {
          console.error("[formatting-enhance-stream] setup error:", e);
